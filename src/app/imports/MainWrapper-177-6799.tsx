import svgPaths from "./svg-72cnzp24rc";

function Frame14() {
  return (
    <div className="content-stretch flex items-center justify-center pl-[12px] relative shrink-0">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-none relative shrink-0 text-[#161a1c] text-[16px] tracking-[0.08px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Settings
      </p>
    </div>
  );
}

function Frame13() {
  return (
    <div className="content-stretch flex items-center justify-center pl-[12px] relative shrink-0">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] opacity-60 relative shrink-0 text-[#161a1c] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Account Settings
      </p>
    </div>
  );
}

function Frame21() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="user-alt">
            <div className="absolute inset-[11.25%_17.5%_10%_17.5%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.1 11.025">
                <path d={svgPaths.p2ebe5700} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        General
      </p>
    </div>
  );
}

function Frame22() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="shield-alt">
            <div className="absolute inset-[10%_12.48%_10.44%_12.52%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 10.5 11.1388">
                <path d={svgPaths.paa14080} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Security
      </p>
    </div>
  );
}

function Frame23() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="bell">
            <div className="absolute inset-[10%_15.02%_10%_15%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.79781 11.2">
                <path d={svgPaths.p2afb5ef0} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Notifications
      </p>
    </div>
  );
}

function Frame12() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame21 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame22 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame23 />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Items() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full" data-name="Items">
      <Frame13 />
      <Frame12 />
    </div>
  );
}

function Frame15() {
  return (
    <div className="content-stretch flex items-center justify-center pl-[12px] relative shrink-0">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] opacity-60 relative shrink-0 text-[#161a1c] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        User Management
      </p>
    </div>
  );
}

function Frame24() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="users">
            <div className="absolute inset-[12.5%_0_15%_0]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 10.15">
                <path d={svgPaths.p170b0000} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Agents
      </p>
    </div>
  );
}

function Frame25() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="contact-card">
            <div className="absolute inset-[15%_5%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.6 9.8">
                <path d={svgPaths.p3b9d1b00} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Roles and Permissions
      </p>
    </div>
  );
}

function Frame16() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame24 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame25 />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Items1() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full" data-name="Items">
      <Frame15 />
      <Frame16 />
    </div>
  );
}

function Frame17() {
  return (
    <div className="content-stretch flex items-center justify-center pl-[12px] relative shrink-0">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] opacity-60 relative shrink-0 text-[#161a1c] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Admin
      </p>
    </div>
  );
}

function Frame26() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="building">
            <div className="absolute inset-[10%_20%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.4 11.2">
                <path d={svgPaths.p12342400} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Organization Profile
      </p>
    </div>
  );
}

function Frame27() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="file-shield">
            <div className="absolute inset-[10%_5%_4.98%_10%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.9 11.9022">
                <path d={svgPaths.pc5527c0} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Data Retention
      </p>
    </div>
  );
}

function Frame28() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="coins">
            <div className="absolute inset-[10%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
                <path d={svgPaths.p3587ca70} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Billing and Usage
      </p>
    </div>
  );
}

function Frame29() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="flag-checkered">
            <div className="absolute inset-[10%_15%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.8 11.2">
                <path d={svgPaths.pff26b80} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        System Status
      </p>
    </div>
  );
}

function Frame18() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame26 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame27 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame28 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame29 />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Items2() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full" data-name="Items">
      <Frame17 />
      <Frame18 />
    </div>
  );
}

function Frame19() {
  return (
    <div className="content-stretch flex items-center justify-center pl-[12px] relative shrink-0">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] opacity-60 relative shrink-0 text-[#161a1c] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Extensions
      </p>
    </div>
  );
}

function Frame33() {
  return (
    <div className="content-stretch flex items-center relative shrink-0 w-full">
      <Frame19 />
    </div>
  );
}

function Frame30() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="sitemap">
            <div className="absolute inset-[15%_10%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 9.8">
                <path d={svgPaths.p2ecc3500} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Integrations
      </p>
    </div>
  );
}

function Frame31() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="chain">
            <div className="absolute inset-[15%_5%_14.97%_5%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.6 9.80438">
                <path d={svgPaths.p386867c0} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Webhooks
      </p>
    </div>
  );
}

function Frame32() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[4px] items-center min-h-px min-w-px relative">
      <div className="content-stretch flex flex-col items-start p-[4px] relative shrink-0" data-name="Dash Icon">
        <div className="relative shrink-0 size-[14px]" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="key">
            <div className="absolute inset-[10%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
                <path d={svgPaths.p2d5d1500} fill="var(--fill-0, #01B386)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#7a8890] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        API Keys
      </p>
    </div>
  );
}

function Frame20() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame30 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame31 />
            </div>
          </div>
        </div>
      </div>
      <div className="content-stretch flex h-[38px] items-start relative shrink-0" data-name="Mini Sidebar Menu">
        <div className="bg-[#f4f7f8] relative rounded-[8px] self-stretch shrink-0 w-[218px]" data-name="Sidebar Menu">
          <div className="flex flex-row items-center size-full">
            <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
              <Frame32 />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Items3() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full" data-name="Items">
      <Frame33 />
      <Frame20 />
    </div>
  );
}

function Frame9() {
  return (
    <div className="content-stretch flex gap-[12px] items-center relative shrink-0">
      <button className="block cursor-pointer opacity-30 overflow-clip relative shrink-0 size-[20px]" data-name="Arrows">
        <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[16px] top-1/2" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="arrow-left">
            <div className="absolute inset-[19.99%_10%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.7994 9.60375">
                <path d={svgPaths.p7993200} fill="var(--fill-0, #6A7282)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </button>
      <div className="opacity-30 overflow-clip relative shrink-0 size-[20px]" data-name="Arrows">
        <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[16px] top-1/2" data-name="Icon">
          <div className="absolute inset-0 overflow-clip" data-name="arrow-right">
            <div className="absolute inset-[19.99%_10%]" data-name="Vector">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.7994 9.60375">
                <path d={svgPaths.p2ea90480} fill="var(--fill-0, #6A7282)" id="Vector" />
              </svg>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame7() {
  return (
    <div className="content-stretch flex gap-[8px] items-center justify-center relative shrink-0">
      <p className="font-['Instrument_Sans:Bold',sans-serif] font-bold leading-[1.2] relative shrink-0 text-[#1e2939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Settings
      </p>
    </div>
  );
}

function Frame8() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0">
      <Frame9 />
      <Frame7 />
    </div>
  );
}

function Frame49() {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <p className="font-['Instrument_Sans:Bold',sans-serif] font-bold leading-[1.2] relative shrink-0 text-[#1e2939] text-[20px] tracking-[0.1px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Recents
      </p>
      <div className="relative shrink-0 size-[16px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="close">
          <div className="absolute inset-[19.99%_19.91%_19.99%_20.07%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.60375 9.60375">
              <path d={svgPaths.p267b3680} fill="var(--fill-0, #6A7282)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame38() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame39() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame38 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Yesterday, 12:56
      </p>
    </div>
  );
}

function Frame40() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        Pauline Nyaboke
      </p>
      <Frame39 />
    </div>
  );
}

function Frame41() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame40 />
    </div>
  );
}

function Frame46() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame45() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame46 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Yesterday, 11:02
      </p>
    </div>
  );
}

function Frame44() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        John Makacha
      </p>
      <Frame45 />
    </div>
  );
}

function Frame42() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame44 />
      <div className="relative shrink-0 size-[16px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="caret-right">
          <div className="absolute inset-[19.99%_29.93%_19.99%_35.08%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 5.59938 9.60325">
              <path d={svgPaths.p25df1f80} fill="var(--fill-0, #6A7282)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame52() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame50() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame52 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Mon, 12:56
      </p>
    </div>
  );
}

function Frame48() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        +254 222 222 222
      </p>
      <Frame50 />
    </div>
  );
}

function Frame47() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame48 />
    </div>
  );
}

function Frame56() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame55() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame56 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Fri, 12:56
      </p>
    </div>
  );
}

function Frame54() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        +1 234 567 8901
      </p>
      <Frame55 />
    </div>
  );
}

function Frame53() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame54 />
    </div>
  );
}

function Frame60() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame59() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame60 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        20/01/2025, 12:56
      </p>
    </div>
  );
}

function Frame58() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        Pauline Nyaboke
      </p>
      <Frame59 />
    </div>
  );
}

function Frame57() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame58 />
    </div>
  );
}

function Frame64() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame63() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame64 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        20/01/2025, 12:56
      </p>
    </div>
  );
}

function Frame62() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        +91 233 4223 442
      </p>
      <Frame63 />
    </div>
  );
}

function Frame61() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame62 />
    </div>
  );
}

function Frame68() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame67() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame68 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        19/01/2025, 12:56
      </p>
    </div>
  );
}

function Frame66() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        Jason Stingo
      </p>
      <Frame67 />
    </div>
  );
}

function Frame65() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame66 />
    </div>
  );
}

function Frame72() {
  return (
    <div className="content-stretch flex items-center relative shrink-0">
      <div className="relative shrink-0 size-[12px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="clock-four">
          <div className="absolute inset-[10%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9.6 9.6">
              <path d={svgPaths.p37b34800} fill="var(--fill-0, #A9BCC6)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] text-right tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        12:22
      </p>
    </div>
  );
}

function Frame71() {
  return (
    <div className="content-stretch flex gap-[13px] items-center relative shrink-0 w-full">
      <Frame72 />
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#a9bcc6] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        18/01/2025, 12:56
      </p>
    </div>
  );
}

function Frame70() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <p className="font-['Instrument_Sans:SemiBold',sans-serif] font-semibold leading-[1.2] relative shrink-0 text-[#1e2939] text-[12px] tracking-[0.06px] w-full" style={{ fontVariationSettings: "'wdth' 100" }}>
        Pauline Nyaboke
      </p>
      <Frame71 />
    </div>
  );
}

function Frame69() {
  return (
    <div className="content-stretch flex gap-[24px] items-center relative shrink-0 w-full">
      <Frame70 />
    </div>
  );
}

function Frame43() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full">
      <Frame41 />
      <Frame42 />
      <Frame47 />
      <Frame53 />
      <Frame57 />
      <Frame61 />
      <Frame65 />
      <Frame69 />
    </div>
  );
}

function Frame51() {
  return (
    <div className="relative shrink-0 w-full">
      <div className="flex flex-col items-center size-full">
        <div className="content-stretch flex flex-col gap-[24px] items-center p-[16px] relative w-full">
          <Frame49 />
          <Frame43 />
        </div>
      </div>
    </div>
  );
}

function Frame34() {
  return (
    <div className="bg-[#f1f4f6] content-stretch flex flex-[1_0_0] flex-col h-[62px] items-center justify-center min-h-px min-w-px relative rounded-[8px]">
      <div className="relative shrink-0 size-[16px]" data-name="clock-rotate-left">
        <div className="absolute inset-[10%_10%_10%_7.5%]" data-name="Vector">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 13.2 12.8">
            <path d={svgPaths.p195c8172} fill="var(--fill-0, #6A7282)" id="Vector" />
          </svg>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Medium',sans-serif] font-medium leading-[1.2] relative shrink-0 text-[#6a7282] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Recents
      </p>
    </div>
  );
}

function Frame36() {
  return (
    <button className="content-stretch cursor-pointer flex flex-[1_0_0] flex-col h-[62px] items-center justify-center min-h-px min-w-px relative rounded-[8px]">
      <div className="relative shrink-0 size-[16px]" data-name="dialpad">
        <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 32 32">
          <g id="Vector" />
        </svg>
        <div className="-translate-x-1/2 -translate-y-1/2 absolute h-[12px] left-1/2 top-1/2 w-[8.727px]" data-name="Vector">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.72727 12">
            <path d={svgPaths.p19630500} fill="var(--fill-0, #292F32)" id="Vector" />
          </svg>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Medium',sans-serif] font-medium leading-[1.2] relative shrink-0 text-[#6a7282] text-[14px] text-left tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Dialer
      </p>
    </button>
  );
}

function Frame35() {
  return (
    <button className="content-stretch cursor-pointer flex flex-[1_0_0] flex-col h-[62px] items-center justify-center min-h-px min-w-px relative rounded-[8px]">
      <div className="relative shrink-0 size-[16px]" data-name="user-group">
        <div className="absolute inset-[12.5%_5%_10%_5%]" data-name="Vector">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14.4 12.4">
            <path d={svgPaths.p39c97100} fill="var(--fill-0, #6A7282)" id="Vector" />
          </svg>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Medium',sans-serif] font-medium leading-[1.2] relative shrink-0 text-[#6a7282] text-[14px] text-left tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Contacts
      </p>
    </button>
  );
}

function Frame37() {
  return (
    <div className="bg-white relative shrink-0 w-full">
      <div aria-hidden="true" className="absolute border-[#f1f4f6] border-solid border-t inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex gap-[8px] items-center p-[8px] relative w-full">
          <Frame34 />
          <Frame36 />
          <Frame35 />
        </div>
      </div>
    </div>
  );
}

function Call() {
  return (
    <div className="absolute bg-[#f4f7f8] content-stretch flex items-center left-0 p-[8px] rounded-[999px] top-0" data-name="call">
      <div className="relative shrink-0 size-[16px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="phone">
          <div className="absolute inset-[10%_9.99%_10.01%_10.01%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.7993 12.7991">
              <path d={svgPaths.p82520f0} fill="var(--fill-0, #4F595E)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}

function Frame1() {
  return (
    <div className="bg-[#f4f7f8] content-stretch flex items-center p-[8px] relative rounded-[999px] shrink-0">
      <div className="opacity-60 overflow-clip relative shrink-0 size-[16px]" data-name="notifications">
        <div className="absolute inset-[10%_15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2001 12.8">
            <path d={svgPaths.p247d8900} fill="var(--fill-0, #1E2939)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Group() {
  return (
    <div className="absolute inset-[6.28%_4.64%]" data-name="Group">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 32.6561 13.3542">
        <g id="Group">
          <g id="Group_2">
            <path d={svgPaths.p11532600} fill="var(--fill-0, #262261)" id="Vector" />
            <path d={svgPaths.p387bb300} fill="var(--fill-0, #262261)" id="Vector_2" />
            <path d={svgPaths.p399da900} fill="var(--fill-0, #262261)" id="Vector_3" />
            <path d={svgPaths.p18463240} fill="var(--fill-0, #262261)" id="Vector_4" />
          </g>
          <g id="Group_3">
            <path d={svgPaths.pb0c6400} fill="var(--fill-0, #67CCE8)" id="Vector_5" />
            <path d={svgPaths.p20dec500} fill="var(--fill-0, #67CCE8)" id="Vector_6" />
          </g>
          <path d={svgPaths.p30c8d280} fill="var(--fill-0, #67CCE8)" id="Vector_7" />
        </g>
      </svg>
    </div>
  );
}

function Frame() {
  return (
    <div className="content-stretch flex gap-[16px] items-center relative shrink-0">
      <div className="relative shrink-0 size-[32px]" data-name="call-button">
        <div className="absolute bg-white content-stretch flex flex-col items-center justify-between left-0 overflow-clip rounded-[15px] shadow-[0px_8px_16px_0px_rgba(0,0,0,0.12)] size-0 top-0" data-name="Call_UI">
          <Frame51 />
          <Frame37 />
        </div>
        <Call />
      </div>
      <Frame1 />
      <button className="block cursor-pointer relative shrink-0 size-[40px]" data-name="Avatar-Popout">
        <div className="absolute bg-[#f3f4f6] content-stretch flex flex-col items-center justify-center px-[3px] py-[4px] right-0 rounded-[8px] size-[40px] top-0" data-name="Avatar-Popout/org/no">
          <div aria-hidden="true" className="absolute border border-[#d1d5dc] border-solid inset-0 pointer-events-none rounded-[8px]" />
          <div className="h-[15.273px] relative shrink-0 w-[36px]" data-name="logo">
            <Group />
          </div>
        </div>
      </button>
    </div>
  );
}

function Frame6() {
  return (
    <div className="content-stretch flex flex-[1_0_0] items-center justify-end min-h-px min-w-px relative">
      <Frame />
    </div>
  );
}

function TabContents() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative" data-name="Tab Contents">
      <div className="flex flex-row items-center justify-center overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex gap-[4px] h-full items-center justify-center py-[14px] relative">
          <div className="flex flex-col font-['Instrument_Sans:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#7a8890] text-[14px] text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">Overview</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function StateLayer() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative w-full" data-name="State-layer">
      <div className="flex flex-col items-center justify-end overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex flex-col items-center justify-end px-[16px] relative size-full">
          <TabContents />
        </div>
      </div>
    </div>
  );
}

function TabContents1() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative" data-name="Tab Contents">
      <div className="flex flex-row items-center justify-center overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex gap-[4px] h-full items-center justify-center py-[14px] relative">
          <div className="flex flex-col font-['Instrument_Sans:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#7a8890] text-[14px] text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">Usage</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function StateLayer1() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative w-full" data-name="State-layer">
      <div className="flex flex-col items-center justify-end overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex flex-col items-center justify-end px-[16px] relative size-full">
          <TabContents1 />
        </div>
      </div>
    </div>
  );
}

function Indicator() {
  return (
    <div className="absolute bottom-0 h-[14px] left-0 right-0" data-name="Indicator">
      <div className="absolute bg-[#0320f5] bottom-0 h-[3px] left-[2px] right-[2px] rounded-tl-[100px] rounded-tr-[100px]" data-name="Shape" />
    </div>
  );
}

function TabContents2() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative" data-name="Tab Contents">
      <div className="flex flex-row items-center justify-center overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex gap-[4px] h-full items-center justify-center py-[14px] relative">
          <div className="flex flex-col font-['Instrument_Sans:SemiBold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#0320f5] text-[14px] text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">Billing History</p>
          </div>
          <Indicator />
        </div>
      </div>
    </div>
  );
}

function StateLayer2() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative w-full" data-name="State-layer">
      <div className="flex flex-col items-center justify-end overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex flex-col items-center justify-end px-[16px] relative size-full">
          <TabContents2 />
        </div>
      </div>
    </div>
  );
}

function TabGroup() {
  return (
    <div className="content-stretch flex items-center relative shrink-0" data-name="Tab group">
      <div className="content-stretch flex flex-col h-[48px] items-center justify-end overflow-clip relative shrink-0 w-[80px]" data-name="Tab-base">
        <StateLayer />
      </div>
      <div className="content-stretch flex flex-col h-[48px] items-center justify-end overflow-clip relative shrink-0 w-[80px]" data-name="Tab-base">
        <StateLayer1 />
      </div>
      <div className="content-stretch flex flex-col h-[48px] items-center justify-end overflow-clip relative shrink-0 w-[80px]" data-name="Tab-base">
        <StateLayer2 />
      </div>
    </div>
  );
}

function Frame2() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0">
      <p className="absolute font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] left-[-15.5px] opacity-0 text-[#99a1af] text-[12px] text-left top-0 tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Search...
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="search">
        <div className="absolute inset-[10%_10%_9.99%_10%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 12.7994 12.8019">
            <path d={svgPaths.p26250ec0} fill="var(--fill-0, #364153)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame3() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0">
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="react-icons/IoFilter">
        <div className="-translate-x-1/2 absolute aspect-[11.25/6.375] bottom-[26.39%] left-1/2 top-[26.39%]" data-name="Vector">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 13.3333 7.55556">
            <path d={svgPaths.pb4b0080} fill="var(--fill-0, #6A7282)" id="Vector" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Filter() {
  return (
    <button className="bg-[#f1f4f6] content-stretch cursor-pointer flex gap-[8px] items-center p-[8px] relative rounded-[8px] shrink-0" data-name="Filter">
      <Frame3 />
    </button>
  );
}

function Frame4() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0">
      <div className="opacity-60 overflow-clip relative shrink-0 size-[16px]" data-name="calendar">
        <div className="absolute inset-[10%_15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 12.8">
            <path d={svgPaths.p28b70d70} fill="var(--fill-0, #6A7282)" id="Primary" />
          </svg>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#364153] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        1 - 10 of 213
      </p>
    </div>
  );
}

function DateFilter() {
  return (
    <div className="bg-white content-stretch flex gap-[8px] items-center px-[10px] py-[8px] relative rounded-[8px] shrink-0" data-name="Date Filter">
      <div aria-hidden="true" className="absolute border border-[#d1d5dc] border-solid inset-0 pointer-events-none rounded-[8px]" />
      <Frame4 />
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="caret-down">
        <div className="absolute inset-[39.98%_24.99%_30%_24.99%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.00333 4.80188">
            <path d={svgPaths.p30cc3700} fill="var(--fill-0, #99A1AF)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame10() {
  return (
    <div className="content-stretch flex flex-[1_0_0] gap-[6px] items-center min-h-px min-w-px relative">
      <button className="bg-[#f1f4f6] cursor-pointer flex-[1_0_0] max-w-[32px] min-h-px min-w-[32px] relative rounded-[8px]" data-name="search_bar">
        <div className="flex flex-row items-center max-w-[inherit] min-w-[inherit] size-full">
          <div className="content-stretch flex items-center max-w-[inherit] min-w-[inherit] p-[8px] relative w-full">
            <Frame2 />
          </div>
        </div>
      </button>
      <Filter />
      <DateFilter />
    </div>
  );
}

function Frame5() {
  return (
    <div className="content-stretch flex gap-[4px] items-center relative shrink-0">
      <div className="relative shrink-0 size-[14px]" data-name="Icon">
        <div className="absolute inset-0 overflow-clip" data-name="arrow-right-to-file">
          <div className="absolute inset-[10%_20%_9.98%_20%]" data-name="Vector">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.4 11.2022">
              <path d={svgPaths.p2440e580} fill="var(--fill-0, #6A7282)" id="Vector" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#364153] text-[12px] tracking-[0.06px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        Export Report
      </p>
    </div>
  );
}

function Frame1070TicketId() {
  return (
    <div className="flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="Frame 1070 ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Medium',sans-serif] font-medium leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            Receipt
          </p>
          <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Sort Icon">
            <div className="absolute inset-[62.5%_29.17%_16.67%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p37f14980} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
            <div className="absolute inset-[16.67%_29.17%_62.5%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p309e1900} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated() {
  return (
    <div className="flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Medium',sans-serif] font-medium leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            Billing Date
          </p>
          <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Sort Icon">
            <div className="absolute inset-[62.5%_29.17%_16.67%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p37f14980} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
            <div className="absolute inset-[16.67%_29.17%_62.5%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p309e1900} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned() {
  return (
    <div className="h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Medium',sans-serif] font-medium leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            Amount
          </p>
          <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Sort Icon">
            <div className="absolute inset-[62.5%_29.17%_16.67%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p37f14980} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
            <div className="absolute inset-[16.67%_29.17%_62.5%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p309e1900} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated1() {
  return (
    <div className="h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Medium',sans-serif] font-medium leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            Actions
          </p>
          <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Sort Icon">
            <div className="absolute inset-[62.5%_29.17%_16.67%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p37f14980} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
            <div className="absolute inset-[16.67%_29.17%_62.5%_29.17%]" data-name="Vector">
              <div className="absolute inset-[-15%_-7.5%]">
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.66667 4.33333">
                  <path d={svgPaths.p309e1900} id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function TableHeading() {
  return (
    <div className="content-stretch flex h-[33px] items-center relative shrink-0 w-full" data-name="table_heading">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <Frame1070TicketId />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated1 />
      </div>
    </div>
  );
}

function TableCellTicketId() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated2() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            November 7, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned1() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated3() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton />
        </div>
      </div>
    </div>
  );
}

function TableRow() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated2 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned1 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated3 />
      </div>
    </div>
  );
}

function TableCellTicketId1() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated4() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            December 29, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned2() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton1() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated5() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton1 />
        </div>
      </div>
    </div>
  );
}

function TableRow1() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId1 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated4 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned2 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated5 />
      </div>
    </div>
  );
}

function TableCellTicketId2() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated6() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            May 29, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned3() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton2() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated7() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton2 />
        </div>
      </div>
    </div>
  );
}

function TableRow2() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId2 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated6 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned3 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated7 />
      </div>
    </div>
  );
}

function TableCellTicketId3() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated8() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            November 28, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned4() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton3() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated9() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton3 />
        </div>
      </div>
    </div>
  );
}

function TableRow3() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId3 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated8 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned4 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated9 />
      </div>
    </div>
  );
}

function TableCellTicketId4() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated10() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            August 24, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned5() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton4() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated11() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton4 />
        </div>
      </div>
    </div>
  );
}

function TableRow4() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId4 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated10 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned5 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated11 />
      </div>
    </div>
  );
}

function TableCellTicketId5() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated12() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            October 24, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned6() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton5() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated13() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton5 />
        </div>
      </div>
    </div>
  );
}

function TableRow5() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId5 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated12 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned6 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated13 />
      </div>
    </div>
  );
}

function TableCellTicketId6() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated14() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            May 9, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned7() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton6() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated15() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton6 />
        </div>
      </div>
    </div>
  );
}

function TableRow6() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId6 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated14 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned7 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated15 />
      </div>
    </div>
  );
}

function TableCellTicketId7() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated16() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            October 24, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned8() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton7() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated17() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton7 />
        </div>
      </div>
    </div>
  );
}

function TableRow7() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId7 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated16 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned8 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated17 />
      </div>
    </div>
  );
}

function TableCellTicketId8() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated18() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            May 9, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned9() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton8() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated19() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton8 />
        </div>
      </div>
    </div>
  );
}

function TableRow8() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId8 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated18 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned9 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated19 />
      </div>
    </div>
  );
}

function TableCellTicketId9() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated20() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            October 24, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned10() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton9() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated21() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton9 />
        </div>
      </div>
    </div>
  );
}

function TableRow9() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId9 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated20 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned10 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated21 />
      </div>
    </div>
  );
}

function TableCellTicketId10() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated22() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            August 24, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned11() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton10() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated23() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton10 />
        </div>
      </div>
    </div>
  );
}

function TableRow10() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId10 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated22 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned11 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated23 />
      </div>
    </div>
  );
}

function TableCellTicketId11() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated24() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            October 24, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned12() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton11() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated25() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton11 />
        </div>
      </div>
    </div>
  );
}

function TableRow11() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId11 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated24 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned12 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated25 />
      </div>
    </div>
  );
}

function TableCellTicketId12() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated26() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            May 9, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned13() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton12() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated27() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton12 />
        </div>
      </div>
    </div>
  );
}

function TableRow12() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId12 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated26 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned13 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated27 />
      </div>
    </div>
  );
}

function TableCellTicketId13() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated28() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            October 24, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned14() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton13() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated29() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton13 />
        </div>
      </div>
    </div>
  );
}

function TableRow13() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId13 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated28 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned14 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated29 />
      </div>
    </div>
  );
}

function TableCellTicketId14() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated30() {
  return (
    <div className="bg-[#f4f4f6] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            May 9, 2024
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned15() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton14() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated31() {
  return (
    <div className="bg-[#f4f4f6] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton14 />
        </div>
      </div>
    </div>
  );
}

function TableRow14() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId14 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated30 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned15 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated31 />
      </div>
    </div>
  );
}

function TableCellTicketId15() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell /  ticket_id">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            6473YEG_298109
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated32() {
  return (
    <div className="bg-[#fefefe] flex-[1_0_0] h-full min-h-px min-w-px relative" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#393939] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            October 24, 2023
          </p>
        </div>
      </div>
    </div>
  );
}

function TableCellAgentAssigned16() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[143px]" data-name="table_cell / agent_assigned">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <div className="flex flex-col font-['Instrument_Sans:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[14px] text-black text-center tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
            <p className="leading-[1.2]">$20.00</p>
          </div>
        </div>
      </div>
    </div>
  );
}

function ActionButton15() {
  return (
    <div className="content-stretch flex gap-[4px] items-center justify-center px-[8px] py-[4px] relative rounded-[8px] shrink-0" data-name="Action Button">
      <p className="font-['Instrument_Sans:Regular',sans-serif] font-normal leading-[1.2] relative shrink-0 text-[#5e6060] text-[14px] tracking-[0.07px] whitespace-nowrap" style={{ fontVariationSettings: "'wdth' 100" }}>
        View
      </p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="Action Icon">
        <div className="absolute inset-[15%]" data-name="Primary">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.2 11.2">
            <path d={svgPaths.p1a597a00} fill="var(--fill-0, #7A7D7D)" id="Primary" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function TableCellDateCreated33() {
  return (
    <div className="bg-[#fefefe] h-full relative shrink-0 w-[120px]" data-name="table_cell / date_created">
      <div aria-hidden="true" className="absolute border-[#e3e3e4] border-r border-solid inset-0 pointer-events-none" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center p-[8px] relative size-full">
          <ActionButton15 />
        </div>
      </div>
    </div>
  );
}

function TableRow15() {
  return (
    <div className="content-stretch flex h-[41px] items-center relative shrink-0 w-full" data-name="table_row">
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellTicketId15 />
      </div>
      <div className="flex flex-[1_0_0] flex-row items-center self-stretch">
        <TableCellDateCreated32 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellAgentAssigned16 />
      </div>
      <div className="flex flex-row items-center self-stretch">
        <TableCellDateCreated33 />
      </div>
    </div>
  );
}

function Table() {
  return (
    <div className="bg-[#fefefe] relative rounded-[6px] shrink-0 w-full" data-name="Table">
      <div className="content-stretch flex flex-col items-start overflow-clip relative rounded-[inherit] w-full">
        <TableHeading />
        <TableRow />
        <TableRow1 />
        <TableRow2 />
        <TableRow3 />
        <TableRow4 />
        <TableRow5 />
        <TableRow6 />
        <TableRow7 />
        <TableRow8 />
        <TableRow9 />
        <TableRow10 />
        <TableRow11 />
        <TableRow12 />
        <TableRow13 />
        <TableRow14 />
        <TableRow15 />
      </div>
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[6px]" />
    </div>
  );
}

function Link() {
  return (
    <div className="bg-[#fefefe] content-stretch flex gap-[4px] h-[36px] items-center opacity-40 px-[8px] relative rounded-[8px] shrink-0" data-name="Link">
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]" />
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="chevron-left">
        <div className="absolute bottom-1/4 left-[37.5%] right-[37.5%] top-1/4" data-name="Vector">
          <div className="absolute inset-[-6.25%_-12.5%]">
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 5 9">
              <path d="M4.5 8.5L0.5 4.5L4.5 0.5" id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      </div>
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">Previous</p>
    </div>
  );
}

function Link1() {
  return (
    <div className="bg-[#e3e3e4] content-stretch flex gap-[4px] items-center justify-center px-[8px] relative rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] shrink-0 size-[36px]" data-name="Link">
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">1</p>
    </div>
  );
}

function Link2() {
  return (
    <div className="bg-[#fefefe] content-stretch flex gap-[4px] items-center justify-center px-[8px] relative rounded-[8px] shrink-0 size-[36px]" data-name="Link">
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]" />
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">2</p>
    </div>
  );
}

function Link3() {
  return (
    <div className="bg-[#fefefe] content-stretch flex gap-[4px] items-center justify-center px-[8px] relative rounded-[8px] shrink-0 size-[36px]" data-name="Link">
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]" />
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">3</p>
    </div>
  );
}

function Link4() {
  return (
    <div className="bg-[#fefefe] content-stretch flex gap-[4px] items-center justify-center px-[8px] relative rounded-[8px] shrink-0 size-[36px]" data-name="Link">
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]" />
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">4</p>
    </div>
  );
}

function Link5() {
  return (
    <div className="bg-[#fefefe] content-stretch flex gap-[4px] items-center justify-center px-[8px] relative rounded-[8px] shrink-0 size-[36px]" data-name="Link">
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]" />
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">5</p>
    </div>
  );
}

function Link6() {
  return (
    <div className="bg-[#fefefe] content-stretch flex gap-[4px] items-center justify-center px-[8px] relative rounded-[8px] shrink-0 size-[36px]" data-name="Link">
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]" />
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">6</p>
    </div>
  );
}

function Frame11() {
  return (
    <div className="content-stretch flex gap-[8px] items-center relative shrink-0">
      <Link1 />
      <Link2 />
      <Link3 />
      <Link4 />
      <Link5 />
      <Link6 />
    </div>
  );
}

function Link7() {
  return (
    <div className="bg-[#fefefe] content-stretch flex gap-[4px] h-[36px] items-center px-[8px] relative rounded-[8px] shrink-0" data-name="Link">
      <div aria-hidden="true" className="absolute border border-[#e3e3e4] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]" />
      <p className="font-['Source_Sans_Pro:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[#202121] text-[13px] text-center whitespace-nowrap">Next</p>
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="chevron-right">
        <div className="absolute bottom-1/4 left-[37.5%] right-[37.5%] top-1/4" data-name="Vector">
          <div className="absolute inset-[-6.25%_-12.5%]">
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 5 9">
              <path d="M0.5 8.5L4.5 4.5L0.5 0.5" id="Vector" stroke="var(--stroke-0, black)" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}

function MainContent() {
  return (
    <div className="absolute content-stretch flex flex-col gap-[24px] items-end left-[19px] top-[88px] w-[899px]" data-name="Main Content">
      <div className="content-stretch flex flex-col items-start relative shrink-0 w-full" data-name="Tabs">
        <TabGroup />
        <div className="content-stretch flex flex-col items-start justify-center relative shrink-0 w-full" data-name="Divider">
          <div className="h-0 relative shrink-0 w-full" data-name="Divider">
            <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 32 32">
              <g id="Divider" />
            </svg>
          </div>
        </div>
      </div>
      <div className="content-stretch flex items-center justify-between relative shrink-0 w-full" data-name="SubHeader">
        <Frame10 />
        <div className="bg-gradient-to-b content-stretch flex from-white items-center px-[10px] py-[8px] relative rounded-[8px] shrink-0 to-[#f1f1f1]" data-name="Button/Dash/Export Report">
          <div aria-hidden="true" className="absolute border border-[#d1d5dc] border-solid inset-0 pointer-events-none rounded-[8px] shadow-[0px_4px_16px_0px_rgba(0,0,0,0.08)]" />
          <Frame5 />
        </div>
      </div>
      <Table />
      <div className="content-stretch flex gap-[16px] items-center justify-end relative shrink-0 w-full" data-name="Pagination">
        <Link />
        <Frame11 />
        <Link7 />
      </div>
    </div>
  );
}

function Main() {
  return (
    <div className="flex-[1_0_0] h-full min-h-px min-w-px overflow-clip relative" data-name="Main">
      <div className="absolute bg-white content-stretch flex h-[64px] items-center justify-between left-0 px-[30px] py-[16px] top-0 w-[1199px]" data-name="Header">
        <div aria-hidden="true" className="absolute border-[#e5e7eb] border-b border-solid inset-0 pointer-events-none" />
        <Frame8 />
        <Frame6 />
      </div>
      <MainContent />
    </div>
  );
}

function Parent() {
  return (
    <div className="bg-white content-stretch flex flex-[1_0_0] items-center min-h-px min-w-px overflow-clip relative rounded-[8px] w-full" data-name="Parent">
      <div className="bg-[#f4f7f8] h-full relative shrink-0 w-[250px]" data-name="side-navigation">
        <div aria-hidden="true" className="absolute border-[#edf1f4] border-r border-solid inset-0 pointer-events-none" />
        <div className="content-stretch flex flex-col gap-[24px] items-start px-[16px] py-[24px] relative size-full">
          <Frame14 />
          <Items />
          <Items1 />
          <Items2 />
          <Items3 />
        </div>
      </div>
      <Main />
    </div>
  );
}

export default function MainWrapper() {
  return (
    <div className="content-stretch flex flex-col items-start pr-[8px] py-[8px] relative size-full" data-name="Main-Wrapper">
      <Parent />
    </div>
  );
}