...
export function QualityRadarChart() {
  ...
  const chartData = useMemo(() => {
    const channelStats: Record<string, { score: number; count: number }> = {};

    // Initialize
    channels.forEach(c => channelStats[c.name] = { score: 0, count: 0 });
    ...
    const subjects = ['Professionalism', 'Quality', 'Timeliness', 'Efficiency', 'Clarity'];
    
    return subjects.map(subject => {
      const row: any = { subject, fullMark: 100 };
      channels.forEach(c => {
        const stats = channelStats[c.name];
        let avg = stats && stats.count > 0 ? stats.score / stats.count : 0;
        ...
        row[c.name] = Math.min(100, Math.max(0, avg + variance));
      });
      return row;
    });
  ...
  return (
    <div className="bg-[#fefefe] rounded-[8px] border border-[#e3e3e4] p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-['Instrument_Sans'] text-[#7a7d7d] text-[14px]">Conversation quality by channel</h3>
        <div className="flex gap-3">
          {channels.map(c => (
            <div key={c.name} className="flex items-center gap-1">
              <div className="w-4 h-2 rounded-[2px] opacity-80" style={{ backgroundColor: c.color }} />
              <span className="font-['Instrument_Sans'] text-[#7a7d7d] text-[12px]">{c.name}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 min-h-[250px] relative">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart cx="50%" cy="50%" outerRadius="70%" data={chartData}>
            ...
            <PolarAngleAxis dataKey="subject" tick={{ fill: 'rgba(0,0,0,0.7)', fontSize: 12, fontFamily: 'Instrument Sans', fontWeight: 600 }} />
            {channels.map(c => (
              <Radar
                key={c.name}
                name={c.name}
