...
function StatusBadge({ label, color, iconPath }: { label: string, color: string, iconPath: string }) {
  return (
    <div className="bg-white box-border flex gap-[4px] items-center justify-center px-[6px] py-[4px] relative rounded-[88px] border border-[#d1d5dc] shadow-[0px_1px_0px_0px_inset_rgba(255,255,255,0.25)]">
      <IconWrapper className="size-[14px]">
        <path d={iconPath} fill={`var(--fill-0, ${color})`} />
      </IconWrapper>
      <p className="font-['Instrument_Sans'] font-semibold leading-[1.2] text-[#364153] text-[12px] tracking-[0.06px]">
        {label}
      </p>
    </div>
  );
...
function ActionButton({ iconPath, label }: { iconPath: string, label?: string }) {
  return (
    <div className={cn("box-border flex gap-[10px] items-center justify-center p-[4px] relative rounded-[400px]", label && "bg-white h-[26px] px-[6px] border border-[#d1d5dc]")}>
      <IconWrapper className="size-[14px]">
        <path d={iconPath} fill="var(--fill-0, #99A1AF)" />
      </IconWrapper>
      {label && (
        <p className="font-['Instrument_Sans'] font-semibold leading-[1.2] text-[#364153] text-[12px] tracking-[0.06px]">
          {label}
...
function ChatHeader() {
  return (
    <div className="bg-white relative w-full border-b border-[#d1d5dc]">
      <div className="box-border flex items-center justify-between px-[16px] py-[6px] w-full">
        <div className="flex gap-[4px] items-center">
          <StatusBadge label="High" color="#FEF9C2" iconPath={svgPaths.p3d6ec2d0} />
          <StatusBadge label="Escalated" color="#8083FF" iconPath={svgPaths.p3c9a6400} />
        </div>
        <div className="flex gap-[4px] items-center">
          <ActionButton iconPath={svgPaths.p2b8aca00} />
          <ActionButton iconPath={svgPaths.pc4343c0} />
          <div className="bg-white box-border flex gap-[4px] h-[26px] items-center justify-center px-[6px] py-[4px] relative rounded-[88px] border border-[#d1d5dc] shadow-[0px_1px_0px_0px_inset_rgba(255,255,255,0.25)] cursor-pointer hover:bg-gray-50">
            <p className="font-['Instrument_Sans'] font-semibold leading-[1.2] text-[#364153] text-[12px] tracking-[0.06px]">
              Assign
            </p>
          </div>
  ...
  return (
    <div className="flex flex-row items-center w-full p-[8px]">
      <div className="flex items-center justify-between w-full">
        <div className="flex gap-[4px] items-center">
          <div className={cn("flex flex-col items-center justify-center rounded-full size-[24px]", isAcmeSupport ? "bg-[#b4bbee]" : "bg-[#f1b2b2]")}>
  ...
  return (
    <div className="bg-[#f2f3f3] rounded-[88px] w-full mt-2">
      <div className="flex items-center justify-between px-[6px] py-[4px] w-full">
        <div className="flex gap-[4px] items-center">
          <IconWrapper className="size-[14px]">
            <path d={svgPaths.p26921480} fill="var(--fill-0, #B1B3B4)" />
          </IconWrapper>
...
function MessageBody({ children, attachment }: { children: React.ReactNode, attachment?: { filename: string, size: string } }) {
  return (
    <div className="bg-white rounded-[13px] w-full">
      <div className="flex flex-col gap-[8px] items-start p-[8px] w-full">
        ...
        {attachment && <Attachment filename={attachment.filename} size={attachment.size} />}
      </div>
    </div>
  );
}
    ...
    <div className="flex flex-row justify-end w-full my-2">
      <div className="flex gap-[6px] items-start justify-end pl-[56px]">
        <div className="flex flex-col gap-[4px] items-end">
          ...
          <div className={cn("box-border flex flex-col gap-[8px] items-start justify-center px-[8px] py-[6px] rounded-[160px]", mentions ? "bg-[#e9e9ff] border border-[#8083ff]" : "bg-[#fcfcfc] border border-[#e3e3e4]")}>
            <div className="font-['Instrument_Sans'] font-normal leading-[1.2] text-[#202121] text-[12px] tracking-[0.06px]">
              {message}
            </div>
          </div>
        </div>
        <div className={cn("flex flex-col items-center justify-center rounded-full size-[17px]", author.includes("Amanda") ? "bg-[#b4bbee]" : "bg-[#f1b2b2]")}>
           {/* Placeholder avatar logic */}
           {author.includes("Amanda") && (
             <div className="text-[10px] font-['Instrument_Sans'] leading-none mt-0.5">A</div>
           )}
           {!author.includes("Amanda") && (
...
function SystemUpdate({ text, tag }: { text: React.ReactNode, tag?: { label: string, color: string, bgColor: string, iconColor: string } }) {
  return (
    <div className="flex gap-[4px] items-center justify-center w-full my-1">
      ...
      {tag && (
        <div className="bg-white box-border flex gap-[2px] items-center justify-center px-[4px] py-[2px] rounded-[6px]" style={{ backgroundColor: tag.bgColor }}>
            {tag.iconColor === "spinner" ? (
                 <Icon12Wrapper className="size-[12px]">
                    <path d={svgPaths.p1718c180} fill="var(--fill-0, #8990DF)" />
                 </Icon12Wrapper>
            ) : tag.iconColor === "chart" ? (
                <Icon12Wrapper className="size-[12px]">
                    <path d={svgPaths.p156e4900} fill="var(--fill-0, #E7A188)" />
                </Icon12Wrapper>
            ) : (
                <Icon12Wrapper className="size-[12px]">
                    <path d={svgPaths.p42f2600} fill={`var(--fill-0, ${tag.iconColor})`} />
                </Icon12Wrapper>
            )}
            <p className="font-['Instrument_Sans'] font-normal text-[#404141] text-[12px]">{tag.label}</p>
        </div>
      )}
    </div>
  );
...
function ReplyBox() {
  return (
    <div className="bg-[#f2f3f3] rounded-[12px] w-full mt-4">
      <div className="flex flex-col gap-[12px] p-[8px] w-full">
        ...
        <div className="bg-[#e3e3e4] flex gap-[2px] items-center p-[4px] rounded-[400px] w-fit">
          <div className="bg-[#fefefe] flex gap-[3px] items-center justify-center px-[6px] py-[2px] rounded-[200px]">
            <p className="font-['Instrument_Sans'] font-semibold text-[#0320f5] text-[12px] tracking-[0.06px]">Reply</p>
          </div>
          <div className="flex gap-[3px] items-center justify-center px-[6px] py-[2px] rounded-[200px]">
            <p className="font-['Instrument_Sans'] font-semibold text-[#7a7d7d] text-[12px] tracking-[0.06px]">Private Comment</p>
          </div>
        </div>
            ...
            <div className="flex gap-[4px] items-center text-[#7A7D7D]">
              <div className="p-[4px] rounded-[4px] hover:bg-gray-200 cursor-pointer">
                <Icon16Wrapper className="size-[16px]">
                    <path d={svgPaths.p25c66f80} fill="currentColor" />
                </Icon16Wrapper>
              </div>
              <div className="p-[4px] rounded-[4px] hover:bg-gray-200 cursor-pointer">
                 <Icon16Wrapper className="size-[16px]">
                     <g>
                        <path d={svgPaths.p2f943480} fill="currentColor" />
                        <path d={svgPaths.p2de75000} fill="currentColor" />
                        <path d={svgPaths.p208c0b40} fill="currentColor" />
                     </g>
                 </Icon16Wrapper>
              </div>
            </div>
            <button className="bg-[#0320f5] flex gap-[4px] items-center justify-center px-[10px] py-[6px] rounded-[4px] hover:bg-blue-700 transition-colors">
              <p className="font-['Instrument_Sans'] font-semibold text-[14px] text-white tracking-[0.07px]">Send</p>
              <Icon16Wrapper className="size-[16px] text-white">
                <path d={svgPaths.p47ef80} fill="currentColor" />
              </Icon16Wrapper>
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-[12px] max-w-[800px] mx-auto">
           ...
           <div className="bg-[#f2f3f3] flex flex-col items-start justify-center p-[4px] rounded-[16px] w-full">
              <MessageHeader 
                ...
                avatarLetter="S" 
              />
              <MessageBody attachment={{ filename: "DSC-20240423092342.png", size: "123KB" }}>
                <p>Hi there, Acme team!</p>
                <br/>
                <p>My account freezes everytime I log in, I keep getting some kind of error and the homepage keeps loading indefinitely.</p>
                <br/>
                <p>See the screenshot attached. I hope you can get it fixed soon because I need to make an important business inquiry.</p>
                <br/>
           ...
           <InternalNote 
             ...
             message={
               <span>
                 Hey <span className="font-semibold text-[#0320f5]">@Amanda Wakio</span>, could you please sort this out?
               </span>
             } 
           />
           ...
           <div className="flex flex-col gap-1 my-2">
              ...
              />
              <SystemUpdate 
                text={<><span className="font-semibold">Jane Muthoni</span> changed priority</>} 
                tag={{ label: "High", color: "#E7A188", bgColor: "#fcf4f2", iconColor: "chart" }} 
              />
              <SystemUpdate 
                text={<><span className="font-semibold">Jane Muthoni</span> changed status</>} 
                tag={{ label: "Escalated", color: "#8990DF", bgColor: "#e9ebfa", iconColor: "spinner" }} 
              />
           </div>
           ...
           */}
           <InternalNoteLeft 
             ...
             message={
               <span>
                 Sure thing <span className="font-semibold text-[#0320f5]">@Jane Muthoni</span>, let me look into it
               </span>
             } 
           />

           <SystemUpdate 
             text={<><span className="font-semibold">Amanda Wakio</span> labelled this conversation</>} 
             tag={{ label: "auth", color: "#8990DF", bgColor: "#e9ebfa", iconColor: "#8990DF" }} 
           />
           ...
           <div className="bg-[#f2f3f3] flex flex-col items-start justify-center p-[4px] rounded-[16px] w-full">
              ...
              <MessageBody>
                <p>Hi Saito,</p>
                <br/>
                <p>Thanks for reaching out. The issue seems to stem from an incorrect configuration in your settings. Let me sort it out for you.</p>
                <br/>
                <p>Warm regards,</p>
                <p>Amanda</p>
              </MessageBody>
